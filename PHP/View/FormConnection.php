...
{
    require adresseRoot.'/Php/View/HtmlConnection.php'; // On affiche le formulaire pseudo password
    ...
    if (empty($_POST['pseudo']) || empty($_POST['password'])) // Oublie d'un champ
    {
        ...
        header("refresh:3;url=Routes.php?action=connect");//connect renvoie à FormConnection
    ...
    } else // On check le mot de passe
    {
        ...
        if ($utilisateur->getPassword() == md5($_POST['password'])) // Acces OK !
        {
            ...
            $_SESSION['level'] = $utilisateur->getLevel();
            $_SESSION['id'] = $utilisateur->getIdUser();
            $_SESSION['group'] = $utilisateur->getIdGroup();
            $message = '<p>Bienvenue ' . $utilisateur->getPseudo() . ', vous êtes maintenant connecté!</p>';
		    echo '<div class="ligne">'.$message.'</div>';
	        header("refresh:1;url=Routes.php?action=mainUser");} 
		else // Acces pas OK !
        {
